  ...
  event.respondWith(caches.open(CACHE_NAME).then(async (cache) => {
    ...
    return cache.match(event.request).then((cachedResponse) => {
      return cachedResponse || fetch(event.request).then((fetchedResponse) => {
          cache.put(event.request, fetchedResponse.clone());  // Add it to cache
          return fetchedResponse;  // Return the network response
        });
    }); 
  }));
  // The above segements is referenced from the demo code open sourced by Chrome Developers. 
  // Citation: Strategies for service worker caching (2021), Chrome Developers, Available at: https://developer.chrome.com/docs/workbox/caching-strategies-overview/
  // Accessed at Nov 14 2022. 
});
